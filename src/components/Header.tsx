...
const Header: React.FC = () => {
  ...
  return (
    <header className={`flex justify-center items-center h-[7vh] text-xl transition-all gap-[3vw] border-b-2 w-[50vw] ${isHomePage ? "fade-header" : ""} ${isTextEvent || isContactPage || isProjectPage ? "pointer-events-auto" : "pointer-events-none"}`}>
      ...
      <div className="relative w-24" onMouseEnter={() => setHoverSchool(true)} onMouseLeave={() => setHoverSchool(false)} onClick={() => setHoverSchool(true)}>
        <div className="relative group flex flex-col items-center cursor-default">
          <p className={`z-10 transition-opacity ${isProjectPage ? "text-[#ffd59e] underline" : ""} ${isTextEvent || isProjectPage || isContactPage ? "mix-blend-difference opacity-100" : "opacity-0"}`}>School</p>
        ...
        <div className={`absolute pt-[5vh] top-0 text-center flex flex-col items-center justify-center right-0 left-0 text-zinc-200 ${isHoverSchool ? "" : "pointer-events-none"}`}>
          <div className={`relative group h-14 flex justify-center items-center border-b-2 border-zinc-300 transition-all duration-500 ${isHoverSchool ? "opacity-100 w-[70%]" : "opacity-0 -translate-y-4 w-0"}`}>
            <p className={`z-10 transition-opacity ${isProjectPage ? "text-[#ffd59e] underline" : ""} ${isTextEvent || isProjectPage || isContactPage ? "mix-blend-difference opacity-100" : "opacity-0"}`}>apagnan</p>
          ...
          <div className={`relative group h-14 flex justify-center items-center border-b-2 border-zinc-300 transition-all duration-500 ${isHoverSchool ? "opacity-100 w-[70%]" : "opacity-0 -translate-y-8 w-0"}`}>
            <p className={`z-10 transition-opacity ${isProjectPage ? "text-[#ffd59e] underline" : ""} ${isTextEvent || isProjectPage || isContactPage ? "mix-blend-difference opacity-100" : "opacity-0"}`}>quoicoubeh</p>
            <span className="absolute h-7 headerHover4 transition-all w-0 group-hover:w-[160%] group-hover:-translate-x-6 duration-[350ms]"></span>
            <span className="absolute h-7 headerHover5 transition-all w-0 group-hover:w-[160%] duration-[250ms]"></span>
            <span className="absolute h-7 headerHover6 transition-all w-0 group-hover:w-[160%] group-hover:translate-x-6"></span>
          </div>
        </div>
      </div>
      <Link to="/contact">
